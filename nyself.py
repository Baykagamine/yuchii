...
def bot(op):
    try:
        ...
        if op.type == 8:
            if wait["autoAdd"] == True:
                cl.findAndAddContactsByMid(op.param1)
                if (wait["recommend"] in [""," ","\n",None]):
                    pass
                else:
                    cl.sendText(op.param1,str(wait["recommend"]))
        if op.type == 13:
                if op.param3 in mid:
        ...
        if op.type == 26:
			...
			#		cl.sendText(msg.to,"Admin permission required.")
                ...
                for manusia in n:
                        cl.sendText(manusia, (bctext))
            elif "Kasihtau " in msg.text:
                if msg.from_ in admin:
                 bctxt = msg.text.replace("Kasihtau ", "")
...
#-----------
					...
					cl.sendMessage(msg)
            elif "Addoac " in msg.text:
				if msg.from_ in admin:
					if msg.toType == 2:
						Midd = msg.text.replace("Addoac ", "")
						t = cl.getAllContactIds()
						for g in t:
							msg.contentType = 13
