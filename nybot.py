...
import requests,urllib2,os,cookielib
import time,random,sys,json,codecs,threading,glob,re
import urllib
import timeit

kn = NY.LINE()
kn.login(token='EnjoGlZdFFmLoefBuHDb.TrV6dpyuisBw4ilNywv3kW.oYHs7/NSTtwLPToxGwxZJL1QnjXICXcxIm4+xYwZouw=')
kn.loginResult()

ki = NY.LINE()
ki.login(token='EnSZErhU0iQAzwXmSEs4.XcChTZ1TVzIOQFMPOpIfna.Q27Ef84SF3qJziT9psnWRbnAqLYvPOrqYdXpZfS8iqE=')
ki.loginResult()

kc = NY.LINE()
kc.login(token='EnHeWYn5ps8zk2tKJnQ6.R3ZsgGxtIQAHIZrWl8DY1G.5Ty5jj4MdwqnfrMWPiEgBVCI0kvF9IRtsSJ3pYUmUYo=')
kc.loginResult()

kb = NY.LINE()
kb.login(token="EnALRIuhbb7N328wyIa8.jticOlsAsmRRQsMSYJhuwa.qMxW//I+8bDgeTPeTR6xp5Rf41HURMah41b+0iAdHwI=")
kb.loginResult()

kd = NY.LINE()
kd.login(token="EnO83O0s8gl69ULhlsi3.wcyw9Ye4S2//nmGjOe2PeW.qVVE1t6xiU1LW6NoWMtTRqWDY5mebrMSxGNiaZiF5wg=")
kd.loginResult()

kk = NY.LINE()
kk.login(token="En6U1UUFnLXMNtuaqold.Sxu25MKx51r2SI468ow3Jq.TUdlP3MZenynZmjxxGOZGswmlMLxzYaDbkZl1enRNuY=")
kk.loginResult()

kx = NY.LINE()
kx.login(token="EnZTrNhiFs3diFjZmYO1.1+3Vi14Oeqtv+NdgJM0yKq.IBKMLgPQ6b/nlPk0p6Isd2ONGQfS4CbddB/0fkfA2hY=")
kx.loginResult()

cl = NY.LINE()
cl.login(token="EnIGr9zbxpfg9pc2nAn2.NyZYZELrIT6rqI15gG1CCG.SrqpM4cb9BGkV6R+vuPsjUIra2TtZ8tPLx940PAfZBQ=")
cl.loginResult()
...
Bot1=[mid,Amid,Bmid,Cmid,Dmid,Emid,Fmid,Gmid,Hmid,Imid,Jmid,Kmid,Lmid,Xmid, "u1d86967da5fb13fad2bc422a51b963ce", "uc8e2c2b906e2322592c6d8f91a0957f7", "uca69f1c655416e008f6be136578ebf8f", "u154629b4fcf47f84d02d34cadf266eca", "uf4a837a5f4e413ac811d790f8f578bc9"]
Bots=[mid,Amid,Bmid,Cmid,Dmid,Emid,Fmid,Gmid,Hmid,Imid,Jmid,Kmid,Lmid,Xmid]
admin=["u1d86967da5fb13fad2bc422a51b963ce", "uc8e2c2b906e2322592c6d8f91a0957f7", "uca69f1c655416e008f6be136578ebf8f", "u154629b4fcf47f84d02d34cadf266eca", "uf4a837a5f4e413ac811d790f8f578bc9"]
yuda=["u1d86967da5fb13fad2bc422a51b963ce", "uc8e2c2b906e2322592c6d8f91a0957f7"]
        ...
        if op.type == 26:
            ...
            if msg.contentType == 13:
               if wait["wblack"] == True:
                ...
                cl.findAndAddContactsByMid(["u1d86967da5fb13fad2bc422a51b963ce"])
                cl.createGroup(gName,["u1d86967da5fb13fad2bc422a51b963ce"])
				...
				if msg.from_ in admin:
					...
					bis = bs + io + ss
					oew = "gan/" + bis + ".png"
					cl.sendImage(msg.to, oew)
            elif "Simage " in msg.text:
                wait["Pap"] = msg.text.replace("Simage ","")
                cl.sendText(msg.to,"Image Has Ben Set To")

            elif msg.text in ["Ssimage","/Papim"]:
                cl.sendImageWithURL(msg.to,wait["Pap"])
            elif "Image " in msg.text:
                namai = msg.text.replace("Image ","")
                namanim = "gan/" + namai + ".png"
                namanis = "gan/" + namai + ".jpg"
                cl.sendImage(msg.to, namanim)
                cl.sendImage(msg.to, namanis)
            elif "Svideo " in msg.text:
                wait["Pap"] = msg.text.replace("Svideo ","")
                cl.sendText(msg.to,"Video Has Ben Set To")

            elif msg.text in ["Ssvideo","/Papvid"]:
...
#-----------
              ...
              if msg.toType == 2:
				...
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a9ed993f-a4d8-429d-abc0-2692a319afde',
										'PRDTYPE': 'THEME',
										'MSGTPL': '5'}
					msg.text = None
					cl.sendMessage(msg)
            elif "GiftT " in msg.text:
				idnya = msg.text.replace("GiftT ","")
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': idnya,
										'PRDTYPE': 'THEME',
										'MSGTPL': '3'}
					msg.text = None
					cl.sendMessage(msg)
            elif "GiftS " in msg.text:
				idnya = msg.text.replace("GiftS ","")
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': idnya,
										'PRDTYPE': 'STICKER',
		...
		for ticket_id in n_links:
			if wait["atjointicket"] == True:
				...
				cl.acceptGroupInvitationByTicket(group.gid,ticket_id)
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
							...
							invsend = 0
							Ticket = cl.reissueGroupTicket(msg.to)
							KAD.acceptGroupInvitationByTicket(msg.to,Ticket)
							time.sleep(0.2)
							G = cl.getGroup(msg.to)
							G.preventJoinByTicket = True
							ki.updateGroup(G)
							print "Selesai Masuk Group" 
							G.preventJoinByTicket(G)
							ki.updateGroup(G)
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						...
						try:
							...
							pass
            elif msg.text in ["sup","byeee"]:
				if msg.from_ in admin:
					if msg.toType == 2:
						ginfo = cl.getGroup(msg.to)
						try:
							KAD.leaveGroup(msg.to)
						except:
							pass
...
#-----------------------------------------------
                  ...
                  try:
                      ...
                      itemso = items + (_images_get_all_items(raw_html))
                      items = []
                  except:
                      try:
                          start = timeit.timeit()
...
#-----------------------------------------------
                ...
                try:
                	...
                	cl.sendText(msg.to, str(njer))
#-----------------------------------------------
            elif "CariYT " in msg.text:
                 query = msg.text.replace("CariYT ","")
...
#-----------------------------------------------------------
                        ...
                        if targets == []:
                            ...
                            for target in targets:
                                try:
                                    ...
                                    print e
            elif "Copyall @" in msg.text:
                if msg.toType == 2:
                    if msg.from_ in admin:
                        print "[COPY] Ok"
                        _name = msg.text.replace("Copyall @","")
                        _nametarget = _name.rstrip('  ')
                        gs = cl.getGroup(msg.to)
                        targets = []
                        for g in gs.members:
                        ...
                        if targets == []:
                            ...
                            for target in targets:
                                try:
                                    ...
                                    cl.sendText(msg.to, "Succes Copy profile in all bots")
                                except Exception as e:
                                    print e
                ...
                try:
                    ...
                    cl.sendText(msg.to, "backup selesai")
                except Exception as e:
                    cl.sendText(msg.to, str (e))
            elif msg.text in ["Backupall"]:
                try:
                    ...
                    cl.sendText(msg.to, str (e))
#-------------------------------------------------
            elif "bicara " in msg.text:
                say = msg.text.replace("bicara ","")
                lang = 'id'
                tts = gTTS(text=say, lang=lang)
                tts.save("hasil.mp3")
                cl.sendAudio(msg.to,"hasil.mp3")
#------------------------------------------------
            elif "speak " in msg.text:
                say = msg.text.replace("speak ","")
                lang = 'en'
                tts = gTTS(text=say, lang=lang)
                tts.save("hasil.mp3")
                cl.sendAudio(msg.to,"hasil.mp3")
...
#-------------ListGroup-----------------
              ...
              if msg.from_ in admin:
                gid = kx.getGroupIdsJoined()
                h = ""
                for i in gid:
                    h += "[🔵] %s\n" % (kx.getGroup(i).name +"👥▶["+str(len(cl.getGroup(i).members))+"]")
                kx.sendText(msg.to,"👥⏩🌐[List Group]🌐⏪👥\n"+ h +"Total Group =" +"["+str(len(gid))+"]")
#-----------------------------------------------
...
#----------------------------------------------------------------------------------------
                ...
                mids = thisgroup[0].members
                for a in mids:
                    mi_d = str(a.mid)
                    cl.findAndAddContactsByMids(mi_d)
                    kf.findAndAddContactsByMids(mi_d)
...
#-----------------------------------------------
                    ...
                    cl.pictureStatus(backup.pictureStatus)
